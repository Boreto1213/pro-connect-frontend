...
const EditClientProfileForm: FC<EditClientProfileFormProps> = ({}) => {
  const { auth } = useAuth()
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<EditClientProfileFormData>({
    resolver: zodResolver(editClientFormSchema),
  })

  const onSubmit = (data: EditClientProfileFormData) => {
    userService
